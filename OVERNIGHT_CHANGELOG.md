...
- Reworked solo auto-start into a persisted `OFF / 0 / 3 / 5 / 10s` cadence. Wave 1 remains manual, automatic calls inside the normal deadline now earn the same +20 as manual calls, the HUD counts down to the configured call rather than the entire intermission, and Settings explains the behavior without the ambiguous `SOLO AFTER WAVE 1` label. Added a focus-safe F11 shortcut through the existing high-DPI fullscreen pipeline.
- Raised Fundamentals' fixed opening-credit compensation from 15% to 25% without changing any tower, enemy, wave, or mid-run economy values. A matched five-seed Hard matrix clears 115/240 (47.9%), still 10.4 points below Standard; Bastion clears 48/240 (20.0%). The decisive ten-seed Surge+Bastion stress case moves from 1/120 with one policy to 6/120 (5.0%) with three policies, preserving a severe challenge while removing the near-impossible outlier.
- Smoothed the excessive Hard-to-Bastion difficulty cliff without changing tower mechanics, waves, or economy: Bastion now uses 112% enemy health, 102% speed, and 18 lives instead of 115% / 104% / 15. A fresh 960-run five-seed audit measures Standard 30.4%, Close Quarters 27.1%, Core Six 15.4%, and Fundamentals 15.4%; an additional 240-run ten-seed Surge stress check confirms Standard at 13.3% and the optional Fundamentals combination at prestige-level 0.8%.
- Replaced the nearly invisible automatic-Protocol pip with an explicit cobalt targeting frame and attached geometric `A` badge that preserves every tower's native identity color and remains visible during activation. The sidebar action now distinguishes `ARM`, `MOVE`, `ON`, and `ARMED` states instead of using one ambiguous `AUTO` label.
- Added a solo Sandbox Lab as a fifth setup mode without polluting competitive balance sweeps, co-op, saves, or run history. It provides unlimited build resources, fixed Base/W10/W20/Immortal enemy health, Standard/Elite/Boss ranks, 1/5/12-target groups, arbitrary arena-wave replay with real difficulty scaling and spawn timing, target clearing, tower-telemetry reset, and immediately repeatable Protocol tests. Immortal targets process real damage/status events while retaining health, so splash, chain, Slow, Burn, Expose, Armor Break, and Protocol effects remain readable outside dense late-game waves.
- Rewrote all forty doctrine/final-role descriptions as compact mechanical language for Tower Intel, removing vague terms such as `lighter`, `harder`, `utility`, and `throughput`. In-match branch buttons now use complete authored names instead of cryptic abbreviations, several opaque names were clarified without changing stable IDs, and exact behavior-aware stat deltas remain directly beneath each plain-language effect summary.
- Simplified live Tower Intel to direct damage, attack rate, and range; derived DPS remains in the Tactical Library where its single-target, pellet, and chain assumptions can be understood. Upgrade hover now uses two compact behavior-aware rows that expose every defining change—including Frost intensity/duration, Needle projectile speed, burn profile, splash cap/homing, armor break, chain reach, Expose duration, shields, and Beacon aura values—while preserving active Beacon/Surge modifiers in the comparison math.
...
- Locked legacy checkpoint compatibility through the current profile and history systems: saves predating difficulty, directive, and run IDs still identify as original Hard/Standard rules and receive a safe run identity without rewriting the source file.
...
- Gave Needle Turret's Rapid Array a visible, bounded ricochet: each primary hit jumps 60% damage to the nearest second enemy within 72 units. This replaces the nearly invisible 16-unit micro-splash while preserving a strict two-target ceiling and Rail Pin's range/armor niche.
...
- Made automatic Protocol behavior role-aware and explicit in planning UI. Tactical Library entries show active duration, cooldown, the exact range/area/priority/cluster/ally trigger, and the engaged elite/boss override. Signal Beacon now activates when either two supported towers are covering threats or one recipient is under six-target pressure; Breaker waits for armored/shielded targets and Mortar waits for a real impact cluster.
...
- Evolved the player-facing No Reserves directive into **Fundamentals** while retaining its stable `no_reserves` identity for save compatibility. Fundamentals keeps all ten towers, disables Plates/Forge plus manual and automatic Protocols at the authoritative session layer, clears prohibited legacy Protocol state on restore, and starts with a fixed 15% credit cushion.
...
- Rebuilt Tactical Library previews around Level 1, two clickable doctrines, and two final-role cards whose exact stats and cumulative costs update under the selected doctrine. In-match Tower Intel uses the same stacked branch controls and labels the active doctrine without covering targeting.
...
- Extended the practical benchmark across all forty doctrine/final-role combinations with moving-rush kills, leaks, health removal, and overkill, making cadence and interception roles visible instead of judging every branch by stationary DPS.
...
- Rechecked the complete native title, Tactical Library, battlefield, Tower Intel, and pause flow at high resolution; moved selected-tower strength/interaction/upgrade lines upward so the `NEXT` preview no longer crowds the Intel card border.
- Expanded the tower reference into a Tactical Library available from title and pause. The Threats tab documents all five base archetypes, exact defenses/economy, recommended counters, Elite/Boss multipliers, the 50% boss phase, and the visual language for Slow, Expose, Armor Break, Burn, and Stun.
...
- Current verification: 68/68 deterministic tests, clean Release build with zero warnings, native high-DPI QA of title/settings/gameplay/pause layouts, real loopback transport coverage, and a 500-tick mid-combat reconnect soak.
...
- Added compact, color-matched route diagrams to every Campaigns-library roster so arena geometry can be compared before starting a run.
...
- Expanded deterministic suite from the original baseline to 68 passing checks.

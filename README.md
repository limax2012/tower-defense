...
Each slot preserves the map, solo/co-op mode, economy and statistics, cleared-wave progress, endless status, towers and their owners/doctrines/final roles/targeting/cooldowns, Protocol state, Pulse Plates, and Charge Forge. Numbered save files are stored under `%LocalAppData%\MinimalBastion\Saves` and are limited in practice only by the filesystem and positive slot-number range. Every overwrite retains one `.bak` recovery generation; if the primary JSON is missing or corrupted, Load Saves and slot metadata transparently use that recovery copy, and a later save never replaces a known-good backup with the corrupt primary. Deleting a slot removes both generations. The host alone writes co-op intermission saves; loading one immediately hosts the restored authoritative state and waits for player 2 to join. An existing legacy `savegame.json` is copied safely into slot 1 when no new slots exist, while the original file remains untouched.

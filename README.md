...
- The title screen keeps six consistent full-width actions. **New Game** and **Online Co-op** open a dedicated setup screen with separate arena, difficulty, and directive cards before play or hosting begins. **Standard** enables every system; **Close Quarters** removes Watchtower/Mortar with 10% more opening credits; **Core Six** is an advanced Needle/Frost/Shard/Ember/Breaker/Beacon roster lock with 30% more opening credits; and **No Reserves** disables Pulse Plates/Forge with 5% more opening credits. Compensation is fixed at session start and never changes tower stats by wave or elapsed time. Directive identity persists through saves, co-op, results, and run history.
...
- Surge Divide is intentionally the hardest arena: its stronger campaign and tighter opening economy pay for nine compact Surge Nodes with focused attack-speed, range, damage, or armor-piercing bonuses. Its route is a slate powered rail with a cyan core and simple moving gold packets; Prism Circuit instead uses a sleeved conduit.
...
Restart requires confirmation, begins a fresh run on the same profile, and selects a new empty autosave slot; it never overwrites the checkpoint being left behind. The confirmation footer states this explicitly in solo and co-op result/pause flows.
